...
( function ( $ ) {
	...
	var imageEdit = ( window.imageEdit = {
		...
		init: function ( postid ) {
			...
			$( 'input[type="text"]', '#imgedit-panel-' + postid ).keypress(
				function ( e ) {
					var k = e.keyCode;
			...
			if ( history !== '' ) {
				...
				for ( n in history ) {
					...
					if ( i.hasOwnProperty( 'c' ) ) {
						op[ n ] = {
							c: { x: i.c.x, y: i.c.y, w: i.c.w, h: i.c.h },
						};
			...
			img = $( '<img id="image-preview-' + postid + '" alt="" />' )
				.on( 'load', { history: data.history }, function ( event ) {
					...
					if ( '' !== event.data.history ) {
						...
						if (
							historyObj[ historyObj.length - 1 ].hasOwnProperty(
								'c'
							)
						) {
							/*
